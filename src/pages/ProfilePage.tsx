...
import { User, Activity, Save, Plus, X, Heart, ShieldAlert } from "lucide-react";
...
export const ProfilePage: React.FC = () => {
  ...
  const handleSaveStats = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess("Metabolic stats and target calories recalculated successfully!");
    } catch (err: any) {
      console.error(err);
  ...
  const saveAllergies = async (updatedList: string[]) => {
    ...
    try {
      ...
      setProfile((prev) => (prev ? { ...prev, allergies: res.allergies } : null));
      showSuccess("Allergies updated. Recipes suggestion AI will reflect these restrictions.");
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="flex-1 flex flex-col px-4 py-6 space-y-6 overflow-y-auto pb-24 max-w-md mx-auto w-full">
      <div className="space-y-1">
        <h2 className="text-2xl font-bold text-white tracking-tight">{t("prof.title", "Your Profile")}</h2>
        ...
        <>
          ...
          <GlassCard className="flex items-center gap-4">
            ...
            <div className="overflow-hidden">
              <h3 className="font-bold text-white leading-tight truncate">{profile.name}</h3>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between px-1">
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
                <Activity className="w-3.5 h-3.5 text-teal-400" /> {t("prof.stats_title", "Metabolic Parameters")}
              </h3>
              <button
                onClick={() => setEditingStats(!editingStats)}
                className="text-xs text-teal-400 hover:text-teal-300 font-semibold transition-colors"
              >
                {editingStats ? "Cancel" : t("prof.edit_btn", "Edit Parameters")}
              </button>
            </div>
            ...
            {editingStats ? (
              <GlassCard className="animate-fade-in">
                <form onSubmit={handleSaveStats} className="space-y-4">
                  {/* Gender */}
                  <div className="space-y-1">
                    <label htmlFor="edit-gender" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.gender", "Gender")}</label>
                    <select
                      id="edit-gender"
                      value={gender}
                    ...
                    >
                      ...
                      <option value="FEMALE">{t("prof.female", "Female")}</option>
                    </select>
                  </div>

                  {/* Birth Date */}
                  <div className="space-y-1">
                    <label htmlFor="edit-birthdate" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.birthdate", "Birth Date")}</label>
                    <input
                      id="edit-birthdate"
                      type="date"
                      value={birthDate}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label htmlFor="edit-height" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.height", "Height (cm)")}</label>
                      <input
                        id="edit-height"
                        type="number"
                        value={height}
                    ...
                    <div className="space-y-1">
                      <label htmlFor="edit-weight" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.weight", "Weight (kg)")}</label>
                      <input
                        id="edit-weight"
                        type="number"
                        value={weight}
                  ...
                  {/* Activity Level */}
                  <div className="space-y-1">
                    <label htmlFor="edit-activity" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.activity", "Activity Level")}</label>
                    <select
                      id="edit-activity"
                      value={activityLevel}
                      ...
                      className="w-full bg-gray-950/40 border border-gray-850 rounded-xl py-2 px-3 text-xs text-white focus:outline-none"
                    >
                      <option value="SEDENTARY">{t("prof.sedentary", "Sedentary")}</option>
                      <option value="LIGHTLY_ACTIVE">{t("prof.light", "Lightly Active")}</option>
                      <option value="MODERATELY_ACTIVE">{t("prof.moderate", "Moderately Active")}</option>
                      <option value="VERY_ACTIVE">{t("prof.very", "Very Active")}</option>
                    </select>
                  </div>

                  {/* Fitness Goal */}
                  <div className="space-y-1">
                    <label htmlFor="edit-goal" className="text-[10px] font-semibold text-gray-500 uppercase">{t("prof.goal", "Goal")}</label>
                    <select
                      id="edit-goal"
                      value={goal}
                      ...
                      className="w-full bg-gray-950/40 border border-gray-850 rounded-xl py-2 px-3 text-xs text-white focus:outline-none"
                    >
                      <option value="LOSE_WEIGHT">{t("prof.lose", "Lose Weight")}</option>
                      <option value="MAINTAIN">{t("prof.maintain", "Maintain")}</option>
                      <option value="GAIN_MUSCLE">{t("prof.gain", "Gain Muscle")}</option>
                    </select>
                  </div>
                  ...
                  >
                    {updatingStats ? (
                      ...
                      <>
                        ...
                        <span>{t("prof.save_btn", "Save & Recalculate Calories")}</span>
                      </>
                    )}
                  </button>
                </form>
              ...
              <GlassCard className="grid grid-cols-3 gap-2 divide-x divide-gray-800/40 text-center py-4">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[9px] text-gray-500 uppercase font-semibold">BMR</span>
                  <span className="text-lg font-bold text-white">{profile.bmr || "—"}</span>
                  <span className="text-[8px] text-gray-400">kcal/day</span>
                </div>
                <div className="flex flex-col gap-0.5 pl-2">
                  <span className="text-[9px] text-gray-500 uppercase font-semibold">TDEE</span>
                  <span className="text-lg font-bold text-white">{profile.tdee || "—"}</span>
                  <span className="text-[8px] text-gray-400">kcal/day</span>
                </div>
                <div className="flex flex-col gap-0.5 pl-2">
                  <span className="text-[9px] text-teal-400 uppercase font-semibold">Target</span>
                  <span className="text-lg font-bold text-teal-400">{profile.targetCal || "—"}</span>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 px-1">
              <ShieldAlert className="w-3.5 h-3.5 text-teal-400" /> {t("prof.allergies_title", "Allergies & Intolerances")}
            </h3>
            ...
            <GlassCard className="space-y-4">
              <div className="flex flex-wrap gap-2">
                {commonAllergies.map((allergy) => {
                  ...
                  return (
                    ...
                    >
                      <Heart className={`w-3.5 h-3.5 ${isSelected ? "fill-red-400/20 text-red-400" : "text-gray-500"}`} />
              ...
              <form onSubmit={handleAddCustomAllergy} className="flex gap-2 border-t border-gray-800/40 pt-3">
                <input
                  type="text"
                  placeholder={t("prof.allergy_placeholder", "Add custom intolerance...")}
                  value={newAllergy}
              ...
              {allergies.length > 0 && (
                <div className="border-t border-gray-800/40 pt-3 space-y-2">
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block">
                    {t("prof.allergy_active", "Allergy Filters Active")} ({allergies.length})
                  </span>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5 px-1">
              <User className="w-3.5 h-3.5 text-teal-400" /> {t("prof.lang_title", "Language / Langue")}
            </h3>
            <GlassCard className="p-4 space-y-3">
              <p className="text-xs text-gray-400">
                {t("prof.lang_sub", "Choose your preferred language for the application interface and AI suggestions (Recipes & Food Analysis).")}
              </p>
